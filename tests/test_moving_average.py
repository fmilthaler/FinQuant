...
def test_EMA():
    orig = np.array([[np.nan, 0.6666666666666666, 1.5555555555555556,
                      ...
                      6.500228623685413, 7.5000762078951375,
                      8.500025402631714],
                     [np.nan, 0.6666666666666666, 2.888888888888889,
                      6.962962962962963, 12.987654320987653,
                      20.99588477366255, 30.998628257887518,
                      42.99954275262917, 56.99984758420972,
                      72.99994919473657]])
    dforig = pd.DataFrame({'0': orig[0], '1': orig[1]}).dropna()
    l1 = range(10)
    l2 = [i**2 for i in range(10)]
...
def test_SMAstd():
    orig = np.array([[np.nan, 0.7071067811865476, 0.7071067811865476,
                      ...
                      0.7071067811865476, 0.7071067811865476,
                      0.7071067811865476],
                     [np.nan, 0.7071067811865476, 2.1213203435596424,
                      3.5355339059327378, 4.949747468305833,
                      6.363961030678928, 7.7781745930520225,
                      9.192388155425117, 10.606601717798213,
                      12.020815280171307]])
    dforig = pd.DataFrame({'0': orig[0], '1': orig[1]}).dropna()
    l1 = range(10)
    l2 = [i**2 for i in range(10)]
...
def test_EMAstd():
    orig = np.array([[np.nan, 0.7071067811865476, 0.9746794344808964,
                      ...
                      1.2219416913579804, 1.2236866244000921,
                      1.2243507269461653],
                     [np.nan, 0.7071067811865476, 2.2693611435820435,
                      4.280032864205755, 6.511621880314852,
                      8.846731940915395, 11.231335395956103,
                      13.640730921938678, 16.063365414263,
                      18.493615652686387]])
    dforig = pd.DataFrame({'0': orig[0], '1': orig[1]}).dropna()
    l1 = range(10)
    l2 = [i**2 for i in range(10)]
...
def test_computeMA():
    stock_orig = [100.0, 0.1531138587991997, 0.6937500710898674,
                  ...
                  0.9996789142433975]
    ma10d_orig = [91.0, 0.12982588130881456, 0.6364686654113839,
                  -0.9111588177100766, -0.45638926346295605,
                  0.22777211487458476, 0.7335679046265856,
                  0.9544686462652832]
    ma30d_orig = [71.0, 0.029843302068984976, 0.40788852852895285,
                  -0.5654851211095089, -0.3735139378462722,
                  0.0648917102227224, 0.4075702001405792,
                  0.5942823972334838]
