...
# create kwargs to be passed to buildPortfolio
...
def test_buildPF_pass_0():
    d = d_pass[0]
    pf1 = buildPortfolio(**d)
    assert(isinstance(pf1, Portfolio))
    assert(isinstance(pf1.getStock(names[0]), Stock))
    ...
    assert(pf1.compWeights() - weights_df_pf <= strong_abse).all()
    pf1.properties()


def test_buildPF_pass_1():
    d=d_pass[1]
    pf2 = buildPortfolio(**d)
    assert(isinstance(pf2, Portfolio))
    assert(isinstance(pf2.getStock(names[0]), Stock))
    ...
    assert(pf2.compWeights() - weights_no_df_pf <= strong_abse).all()
    pf2.properties()


def test_buildPF_pass_2():
    d = d_pass[2]
    pf3 = buildPortfolio(**d)
    assert(isinstance(pf3, Portfolio))
    assert(isinstance(pf3.getStock(names[0]), Stock))
    ...
    assert(pf3.compWeights() - weights_no_df_pf <= strong_abse).all()
    pf3.properties()


def test_buildPF_pass_3():
    d = d_pass[3]
    pf4 = buildPortfolio(**d)
    assert(isinstance(pf4, Portfolio))
    assert(isinstance(pf4.getStock(names[0]), Stock))
    ...
    assert(pf4.compWeights() - weights_no_df_pf <= strong_abse).all()
    pf4.properties()


def test_buildPF_pass_4():
    d = d_pass[4]
    pf5 = buildPortfolio(**d)
    ...
    assert(pf5.compWeights() - weights_df_pf <= strong_abse).all()
    assert(expret_orig - pf5.expectedReturn <= strong_abse)
    assert(vol_orig - pf5.volatility <= strong_abse)
    assert(sharpe_orig - pf5.sharpe <= strong_abse)
    assert(freq_orig - pf5.freq <= strong_abse)
    assert(riskFreeRate_orig - pf5.riskFreeRate <= strong_abse)
    pf5.properties()


####################################################################
# tests that are meant to raise an exception during buildPortfolio #
####################################################################
...
def test_Stock():
    ...
    for i in range(len(pf.stocks)):
        assert(isinstance(pf.getStock(names[0]), Stock))
        stock = pf.getStock(names[i])
...
def test_get_ef():
    ...
    assert(pf.ef == ef)
    assert((pf.compMeanReturns(freq=1) == ef.meanReturns).all())
    assert((pf.compCov() == ef.cov_matrix).all().all())
    assert(pf.freq == ef.freq)
    assert(pf.riskFreeRate == ef.riskFreeRate)
...
def test_ef_minimum_volatility():
    ...
    assert(np.allclose(ef_opt_weights.values.transpose(), min_vol_weights, atol=weak_abse))
...
def test_maximum_sharpe_ratio():
    ...
    assert(np.allclose(ef_opt_weights.values.transpose(), max_sharpe_weights, atol=weak_abse))
...
def test_efficient_return():
    ...
    assert(np.allclose(ef_opt_weights.values.transpose(), efficient_return_weights, atol=weak_abse))
...
def test_efficient_volatility():
    ...
    assert(np.allclose(ef_opt_weights.values.transpose(), efficient_volatility_weights, atol=weak_abse))
