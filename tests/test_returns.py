...
def test_cumulativeReturns():
    ...
    d = {'1': l1, '2': l2}
    df = pd.DataFrame(d)
    ret = cumulativeReturns(df)
    assert(all(abs(ret['1'].values - orig[0]) <= 1e-15))
    assert(all(abs(ret['2'].values - orig[1]) <= 1e-15))
    # with dividend of 0.2
    orig = [[0.2+i for i in range(10)],
            [0.005, -0.02, -0.045, -0.07, -0.095,
             -0.12, -0.145, -0.17, -0.195, -0.22]]
    ret = cumulativeReturns(df, 0.2)
    assert(all(abs(ret['1'].values - orig[0]) <= 1e-15))
    assert(all(abs(ret['2'].values - orig[1]) <= 1e-15))


def test_dailyReturns():
    orig = [[1.0, 1./2, 1./3, 1./4],
        [1./9, 1./10, 1./11, 1./12]]
    l1 = range(1, 6)
...
def test_dailyLogReturns():
    orig = [[0.6931471805599453, 0.4054651081081644,
         0.28768207245178085, 0.22314355131420976],
        [0.10536051565782635, 0.09531017980432493,
         0.0870113769896297, 0.08004270767353636]]
    l1 = range(1, 6)
    l2 = [10*0.2+i*0.25 for i in range(1,6)]
    d = {'1': l1, '2': l2}
    df = pd.DataFrame(d)
    ret = dailyLogReturns(df)
    ret
